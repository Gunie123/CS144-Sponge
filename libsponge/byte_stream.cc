...
ByteStream::ByteStream(const size_t capacity)
    : _buffer(capacity + 1), _capacity(capacity), _written_cnt(0), _read_cnt(0), _head(0), _tail(_capacity) {}
...
string ByteStream::peek_output(const size_t len) const {
    string ret;
    size_t actual_len = min(len, buffer_size());
...
void ByteStream::pop_output(const size_t len) {
    size_t actual_len = min(len, buffer_size());
    _head = (_head + actual_len) % (_capacity + 1);
